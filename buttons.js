function attachButton(li) {
  ...
  // create favorite button svg
  const favButton = document.createElementNS(xmlns, "svg");

  // set dimensions and attributes of check button
  favButton.setAttributeNS(null, "viewBox", "0 0" + " " + boxWidth + " "+ boxHeight);
  favButton.setAttributeNS(null, "width", boxWidth);
  favButton.setAttributeNS(null, "height", boxHeight);
	favButton.setAttributeNS(null, 'class', "fav");
  favButton.setAttributeNS(null, 'preserveAspectRatio', 'xMidYMid');
  let favPath = document.createElementNS (xmlns, "path");

  // draw favorite button svg path
  ...
  favButton.appendChild(favPath);
  btns.appendChild(favButton);

  // set specific width and height for trash can button
	boxWidth = 36;
	boxHeight = 24;

  // set dimensions and attributes of trash bin button
  ...
  // initialize trash bin, cap, and can variables
